...
Очевидно, что в реальности, такое ускорение не достижимо, т.к. цикл будет работать до тех пор, пока все точки не выйдут из множества. Таким образом, некоторые точки будут считаться вхолостую. Поэтому ускорение в 7-8х для `AVX` и 3-4х в `SSE` считаю адекватным.
